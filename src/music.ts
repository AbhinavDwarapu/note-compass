...
export const midiAt = (cell: Cell) => GUITAR_STRINGS[cell.stringIndex].openMidi + cell.fret

export const pitchClassAt = (cell: Cell) => midiAt(cell) % 12

export const isNaturalPitch = (pitchClass: number) => !PITCH_NAMES[pitchClass].includes('♯')
...
export const sameCell = (a: Cell, b: Cell) => a.stringIndex === b.stringIndex && a.fret === b.fret
...
export function buildCandidates(settings: Settings): Cell[] {
  ...
  GUITAR_STRINGS.forEach((_, stringIndex) => {
    ...
    for (let fret = lowestFret; fret <= settings.fretCount; fret++) {
      if (settings.naturalsOnly && !isNaturalPitch(pitchClassAt({ stringIndex, fret }))) continue
...
export function pickWeighted(
  ...
  const pool = cells.length > 1 ? cells.filter((cell) => cellKey(cell) !== avoidKey) : cells
...
export function pickUnlockCell(
  ...
  if (order === 'nut') {
    ...
    locked.forEach((cell) => {
      ...
      if (!current || cell.fret < current.fret) frontierByString.set(cell.stringIndex, cell)
    })
  ...
  const freshPitches = locked.filter((cell) => !unlockedPitches.has(pitchClassAt(cell)))
  const pool = freshPitches.length > 0 ? freshPitches : locked
  return pool[Math.floor(Math.random() * pool.length)]
}

export function withSeeds(settings: Settings, unlockedKeys: string[]): string[] {
  if (settings.mode !== 'incremental') return unlockedKeys
  const universe = buildCandidates(settings)
  const targetCount = Math.min(3, universe.length)
